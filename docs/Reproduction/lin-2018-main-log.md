...
- 2026-04-02：完成非线性层开工前的最后一轮工程收口。`train.py` 正式加入 `--seed` 与 `--experiment-stage`，分类与成像训练的 manifest 现在统一记录实验阶段、随机种子和光学配置；`visualize.py` 新增 `--seed`，并在成像 `imagefolder` 场景下优先从 checkpoint 相邻 manifest 继承训练时的种子，避免训练与可视化测试划分不一致；同时把当前 `5 层 phase-only + Fashion-MNIST` 对照组正式冻结为后续非线性机制与插入位置消融的基线记录，并整理进官方 preserved historical bundle。
...
- 2026-04-11：完成实验室单层 handoff 路径收口。`tasks.py` 已不再依赖 `train.py`，共享分类训练 helper 已收口到 `train_core.py`。`train.py` 新增 `--optics-preset paper|lab852_f10|lab852_f5`，其中实验室 preset 固定为 `wavelength = 852 nm`、`pixel_size = 1 um`。菲涅尔距离说明现已明确采用半孔径 `a = (size * pixel_size) / 2`，在 `size = 200` 下得到 `F≈10 -> z≈1.1737 mm`、`F≈5 -> z≈2.3474 mm`。`visualize.py` 与 `export_phase_plate.py` 现可从 manifest 继承 checkpoint optics；`export_phase_plate.py` 也已正式支持 `--export-bmp`，将 wrap 后的 `[0, 2pi)` 相位线性映射到 `[0, 255]` 灰度。验证结果：`uv run pytest -q` 通过（`139 passed`）。同时已实际跑通一次 smoke run：`uv run python train.py --task classification --dataset fashion-mnist --epochs 1 --size 200 --layers 1 --seed 42 --optics-preset lab852_f10 --experiment-stage lab_single_layer --rs-backend fft --save-dir checkpoints_smoke`，生成的 manifest 记录了 `wavelength = 8.52e-07`、`pixel_size = 1e-06`、`layer_distance = 0.00117370892018779`、`num_layers = 1`。后续 `visualize.py` 与 `export_phase_plate.py --export-bmp` 也已跑通，并生成 `phase_masks.png`、`phase_masks.npy` 与 `bmp/layer_01_phase_8bit.bmp`。当前仓库已经具备单层实验室路径，下一优先级不再是多层重训，而是决定继续拉长单层训练预算，还是直接转入 SLM 侧验证。
- 2026-04-11：在发布就绪 review 之后重新完成了单层 handoff 的验证收口。当前工作树上 `uv run pytest -q` 已通过（`146 passed`）；同时在发布准备阶段补上了 `Pillow` 到 `pyproject.toml`，并将 `checkpoints_smoke*`、`exports_smoke*`、`figures_smoke*` 输出加入忽略规则，从而避免在推送 handoff 代码时把本地 smoke 产物误发到仓库。
- 2026-04-11：已将全局 optics 默认值更新为最新确认的实验室参数：`wavelength = 852e-9`、`pixel_size = 1e-6`、`input_distance = 491.302e-3`、`output_distance = 575.304e-3`；现阶段有意保留原有 `layer_distance` 数值，作为待确认前的临时层间距占位。模型前向路径现已拆分为显式的输入面传播与输出面传播，不再继续把整条光路压成一个共享距离；CLI、manifest 和 export wrapper 也都已记录这两个拆分距离。阶段 review 说明：外部只读 reviewer 在超时前未返回可用结果，因此本阶段采用 fallback 本地 review；当前未发现阻塞性回归，剩余唯一明确歧义是多层 `inter-layer spacing`。验证结果：`uv run pytest -q` 通过（`150 passed`）。
- 2026-04-12：根据独立 review 修复了两处 optics 兼容性风险。其一，esolve_optics() 现已对缺少 input_distance/output_distance 的 legacy manifest 直接 fail fast，不再把历史 checkpoint 静默重解释为新的 split-distance 光路；其二，export_fmnist5_phaseonly_aligned_final.py 的 bootstrap 路径现已要求 source manifest 的 optics 与当前官方 preset 完全一致，若仍是旧 phase_masks + 旧 optics 工件，则拒绝自动重建 checkpoint，避免导出物理上不自洽的混合产物。对应回归测试已补齐并通过，当前完整验证结果为 uv run pytest -q 通过（152 passed）。
- 2026-04-12：已将当前实验室验证路径进一步收口到单层。除了训练入口继续要求 `lab852_*` 只能用于 `classification --layers 1` 之外，`resolve_optics()` 现也会对多层 `lab852_*` checkpoint 直接 fail fast，因此可视化与导出链路不再会误放行多层实验室工件；同时保留底层多层模型实现，作为单层初步可行度验证完成后的后续扩层研究骨架。对应文档已同步说明 `lab852_*` 只代表当前单层实验室路径，而历史 `fmnist5-phaseonly-aligned` 5-layer wrapper 仅作为历史 fabrication baseline 保留。验证结果：`uv run pytest -q` 通过（`154 passed`）。
- 2026-04-12：为当前实验室验证阶段进一步收口单层约束。除训练入口继续要求 lab852_* 只能用于 classification + --layers 1 外，esolve_optics() 现在也会对多层 lab852_* checkpoint 直接 fail fast，因此 isualize.py 与 export_phase_plate.py 不再误放行多层实验室工件；对应回归测试已补到 	ests/test_d2nn_core.py。文档也已同步说明：当前 lab852_* 只代表单层实验室验证路径，底层多层实现保留给后续完成单层初步可行度验证后的扩层研究，历史 mnist5-phaseonly-aligned 5-layer wrapper 仅作为历史 baseline 保留。阶段 review 说明：独立 reviewer 已完成只读复核，未发现新的阻塞性问题。验证结果：uv run pytest -q 通过（154 passed）。
- 2026-04-12：做了一次不改 CLI、不改模型行为的低风险去冗余。当前已把 classification 训练前的共享上下文构建收口到 `tasks.py`，新增 `ClassificationTrainingContext`、`build_classification_training_context()` 和 `build_classification_training_model()`，并删除 `train.py` 中重复的 run-context 拼装逻辑。验证范围保持在小而直接的回归层面：`uv run python -m pytest tests/test_d2nn_core.py -k "classification_training_context or classification_training_model or train_parser_accepts_rs_acceleration_flags or tasks_module_does_not_import_train_textually" -q` 通过，且 `uv run python -m pytest tests/test_d2nn_core.py -k "build_classification_training_context_applies_run_metadata_overrides" -q` 通过。
- 2026-04-12：继续做第二轮 low-risk simplification，把 classification dataset / loader 准备也收口到 `tasks.py`。当前新增 `build_classification_train_val_test_loaders()` 和 `build_classification_test_loader()`，并让训练入口 `train.py` 与分类可视化路径共用这两个 helper，避免在两个文件里重复拼装 transform / dataset_cls / test loader。对应 TDD 验证为：先补 `tests/test_d2nn_core.py` 中的 failing tests，再实现 helper；最终 `uv run python -m pytest tests/test_d2nn_core.py -k "classification_test_loader or classification_train_val_test_loaders" -q` 通过（`2 passed`），且上一轮 shared-context 回归集继续通过（`5 passed`）。
- 2026-04-12：继续做第三轮 low-risk simplification，把 classification manifest payload 的字段拼装也收口到 `tasks.py`。当前新增 `build_classification_manifest_payload()`，并让 `train.py` 的 manifest 落盘直接复用该 helper，从而去掉本地重复的 payload 组装逻辑而不改变字段语义。对应 TDD 验证为：先补 manifest payload 的 failing test，再实现 helper；最终 `uv run python -m pytest tests/test_d2nn_core.py -k "build_classification_manifest_payload_preserves_expected_fields" -q` 通过（`1 passed`），且此前的 shared context / loader 回归集继续通过（`5 passed`）。
- 2026-04-13：继续沿“稳定净删代码”方向削薄 `train.py`，本轮把 classification 训练里的包装层进一步内联，删除了仅单次转发的 epoch-history、best-checkpoint、manifest-save 与 finalize wrapper，保持训练循环、checkpoint 选择与 manifest 字段语义不变。验证结果：`uv run python -m pytest tests/test_d2nn_core.py -k "build_classification_manifest_payload_preserves_expected_fields or classification_training_context or classification_training_model or classification_test_loader or classification_train_val_test_loaders or train_parser_accepts_rs_acceleration_flags or tasks_module_does_not_import_train_textually" -q` 通过（`8 passed`），并已实际跑通一条 classification smoke：`uv run python train.py --task classification --dataset fashion-mnist --epochs 1 --size 200 --layers 1 --seed 42 --optics-preset lab852_f10 --experiment-stage refactor_smoke --run-name refactor_smoke_slim --rs-backend fft --save-dir checkpoints_smoke_reviewed`。阶段审查：已实际使用独立 reviewer agent 做只读复核，结论 `No findings`；剩余风险主要仍是未跑长程完整训练，而非本轮 diff 中可见的行为漂移。
- 2026-04-13：在继续净删 `train.py` 的过程中，做了一次完整 `uv run pytest -q` 作为收口检查。全量测试先暴露出一个真实兼容性回归：我在清理导入时一并移除了 `train.py` 对 `classification_composite_loss` 的模块级导出，导致 `test_train_module_exposes_classification_training_core` 失败。随后已恢复该导出，并重新跑完全量测试，当前结果为 `160 passed`。这次结果说明当前精简可以收口在“训练入口显著变薄，但保留既有模块级兼容面”这一稳定点上。
- 2026-04-16: moved train-only classification helpers back into `train.py` so future debugging stays centered there; kept deleted duplicate code deleted; left only shared classification visualization/config helpers in `tasks.py`. Independent reviewer-agent pass returned `No findings`. Validation: targeted refactor tests passed and `uv run pytest -q` passed (`160 passed`).
- 2026-04-16: continued simplifying `train.py` in place instead of moving logic outward. Reduced classification parameter forwarding by making manifest payload and train/val/test loader helpers consume the training context directly, cut duplicated train/val epoch call structure, removed one unnecessary best-checkpoint reload when the final epoch is already best, and narrowed `fit_classification_model()` to the parameters it actually uses. Self-review found no behavioral drift; validation remained green with targeted refactor tests and `uv run pytest -q` (`160 passed`).
- 2026-04-16: fixed an externally-reviewed regression in `tasks.py` where the classification understanding-report path still referenced deleted `test_set`; the visualization path now uses `test_loader.dataset`, and a regression test now covers `understanding_report=True`. Continued simplifying `train.py` by moving `seed/epochs/batch_size/learning_rate/experiment_stage` into `ClassificationTrainingContext`, so the classification loader, fit loop, and manifest payload helpers depend on context rather than the full CLI args namespace. Validation: targeted refactor/visualization tests passed and `uv run pytest -q` passed (`161 passed`).
- 2026-04-16: continued from structure cleanup into small runtime-efficiency wins inside `train.py`. The classification training loop now keeps an in-memory CPU copy of the best state dict while still saving checkpoints on every improvement, so post-training evaluation no longer needs a disk `torch.load()` when restoring the best model; a regression test now covers this restore path. The best-save branch also stopped calling `model.state_dict()` twice. Validation remained green with targeted restore-path tests and `uv run pytest -q` (`163 passed`).
- 2026-04-16: switched from “reorganizing” to true deletion-oriented simplification in `train.py`. Removed the classification context/dataclass layer plus the thin runtime-config / loader / model-build / manifest-payload / restore helpers, and inlined their logic back into `run_classification_training()` while keeping the training loop helper intact. Deleted the helper-specific tests that only existed for the removed abstractions; `train.py` + corresponding tests are now net negative in LOC, and `uv run pytest -q` still passes (`156 passed`).
- 2026-04-16: follow-up review on the deletion-oriented `train.py` simplification found one real visualization regression earlier (already fixed in `tasks.py`) and one later `epochs=0` concern that was verified against the pre-refactor code path and judged to be preserved historical behavior rather than a new regression. Also removed the single-use `seed_everything()` / `configure_runtime()` helpers by inlining them into `run_training_task()`. Validation remains `uv run pytest -q` passing (`156 passed`).
- 2026-04-16: continued the deletion-oriented `train.py` cleanup without adding new wrappers. Simplified `fit_classification_model()` by removing the per-epoch split-metrics staging dict and keeping train/val metrics as direct locals, and flattened the remaining runtime-argument bridge in `run_classification_training()` by replacing redundant `getattr(...)` reads with direct parser-backed args plus one local `allow_tf32` flag. An independent reviewer re-raised the previously fixed `tasks.py` understanding-report issue, but manual verification of the current worktree confirmed the code already uses `test_loader.dataset`, so there was no live regression to fix in this stage. Validation: targeted train-path tests passed and `uv run pytest -q` passed (`156 passed`).
- 2026-04-16: formalized the current `train.py` cleanup as a Superpowers execution phase with a dedicated plan file at `docs/superpowers/plans/2026-04-16-train-py-net-negative-simplification.md`, then executed the next two deletion-oriented tasks inline. `fit_classification_model()` now tracks best-checkpoint state with direct locals instead of short-lived metric dict churn, and `run_classification_training()` drops a few remaining parser-backed type/bridge redundancies (`int(...)`/`bool(...)` casts, one-shot dataset class staging, one-shot manifest-path staging) without changing CLI or manifest semantics. Stage review: independent reviewer returned `no findings`. Fresh verification: `uv run pytest -q` passed (`156 passed`).
- 2026-04-16: continued the same Superpowers execution path and pushed one more net-negative simplification step into `fit_classification_model()`. The per-split train/val epoch bookkeeping is back to a single inline loop without reviving the old staging dict, and the best-checkpoint rule is now expressed as a lexicographic tuple comparison `(accuracy, contrast, epoch)`, which preserves the same ordering semantics while removing nested conditional noise. Fresh verification: `uv run pytest -q` passed (`156 passed in 19.63s`). Independent reviewer result: `no findings`.
- 2026-04-16: pushed one final low-risk noise-reduction pass into `train.py`. Inlined the one-shot `best_state_dict` branch, removed the one-use `allow_tf32` and `total_params` locals from the classification path, and kept the manifest/runtime payload semantics unchanged. Fresh verification: `uv run pytest -q` passed (`156 passed in 20.10s`). Independent reviewer result: `no findings`, with the additional conclusion that `train.py` is now close to a natural stopping point: the remaining code is mostly the core training flow and CLI entry surface rather than obvious wrapper or parameter-forwarding redundancy.
- 2026-04-16: Task 5 verification for the multilayer training stability plan completed. The stability guards in this phase are the new finite checks inside `_run_classification_epoch()` for `scores`, `logits`, `contrast`, and the loss terms `total`, `mse`, `ce`, `reg`, plus early reference release with `del result, loss_terms, pred` after batch accumulation. `d2nn.py` did not change in this phase. Full-suite validation: `uv run pytest -q` passed with `160 passed, 5 subtests passed in 19.65s`. Smoke validation: `uv run python train.py --task classification --dataset fashion-mnist --epochs 1 --size 200 --layers 1 --seed 42 --optics-preset lab852_f10 --experiment-stage stability_smoke --run-name stability_smoke --rs-backend fft --save-dir checkpoints_smoke_stability` completed successfully, trained one epoch, saved `best_fashion_mnist.stability_smoke.pth`, and reported `Test accuracy: 10.00% | Test contrast: -0.0098`. Independent review is still pending and will be appended by the controller if needed.
- 2026-04-16: final independent review for the multilayer training stability phase returned `NO FINDINGS`. The reviewer rechecked `train_core.py`, `train.py`, and `tests/test_d2nn_core.py`, confirmed that the new fail-fast guards and their coverage are aligned, that `del result, loss_terms, pred` is placed after all required uses and is semantically safe, and that checkpoint/manifest semantics in `train.py` did not drift.
- 2026-04-16: repository outdated-content cleanup logging pass completed after doc cleanup and tracked historical artifact removal. Documentation now clearly demotes the historical 5-layer proxy material and frozen fabrication wrapper by separating active entrypoints from historical baselines in `README.md` and `docs/INDEX.md`; the removed tracked historical artifacts were `checkpoints_proxy/`, `reports/post_rs_eval/`, `reports/post_rs_retrain/`, `reports/post_rs_compressed/`, and `figures/post_rs_compressed/`. Full verification result: `uv run pytest -q` passed (`160 passed, 5 subtests passed in 37.03s`). Independent review is pending and will be appended later by the controller.
- 2026-04-16: final independent review for the repository outdated-content cleanup phase returned `NO FINDINGS`. The reviewer confirmed that `README.md` and `docs/INDEX.md` now present a coherent active-vs-historical split, that `docs/official-artifacts/README.md` and `docs/official-artifacts/fmnist5-phaseonly-aligned/` remain on the active official side, and that deleting `checkpoints_proxy/`, `reports/post_rs_eval/`, `reports/post_rs_retrain/`, `reports/post_rs_compressed/`, and `figures/post_rs_compressed/` did not leave tracked documentation or code-path regressions.
- 2026-04-17: completed the historical 5-layer fabrication cleanup phase. Removed the pure historical 5-layer baseline note and its editable CSV bundle from the main tree, while keeping the preserved official historical bundle discoverable through `docs/official-artifacts/fmnist5-phaseonly-aligned/`, `export_fmnist5_phaseonly_aligned_final.py`, and `fabrication/fmnist5-phaseonly-aligned.lab.template.json`. Updated `README.md`, `docs/INDEX.md`, `docs/Reproduction/README.md`, `docs/fabrication/fashion-mnist-phase-only-lab-handoff.md`, `docs/fabrication/fashion-mnist-phase-only-lightpath-protocol.md`, and this log so the active single-layer path stays first and no stale direct references remain to the removed baseline materials. Full verification result: `uv run pytest -q` passed (`160 passed, 5 subtests passed in 18.72s`). Final independent review returned `NO FINDINGS`.
- 2026-04-20: continued the same entrypoint/layering direction by moving loader runtime policy fully back into `train.py`. Both classification and imaging training now derive `num_workers`, `pin_memory`, `prefetch_factor`, and the manifest `runtime_config` from the unified training entry layer, while `tasks.build_imaging_loaders(...)` consumes that precomputed runtime policy instead of deciding device/prefetch behavior itself. This keeps task helpers focused on dataset/loaders and fit logic, and makes the orchestration skeleton for classification and imaging more alike without re-merging low-level implementation. Verification: targeted boundary tests passed (`5 passed, 155 deselected`), `py -3 -m compileall train.py tasks.py` passed, full validation `uv run pytest -q` passed (`160 passed, 5 subtests passed in 18.46s`), and final independent review returned `NO FINDINGS`.
- 2026-04-20: aligned the save/summary side of the unified training entrypoint without changing task behavior. `train.py` now resolves common run identity (`run_name` + checkpoint path) and common manifest field assembly through shared orchestration helpers for both classification and imaging, while still leaving task-specific metrics and fit logic in the layered modules. During review, an imaging-only summary wording regression was caught because the shared model-summary helper initially reused the classification phrase `neurons/layer`; that wording was restored to be task-specific while keeping the shared orchestration shape. Verification after the fix: targeted tests passed (`3 passed, 157 deselected`), `py -3 -m compileall train.py` passed, and full validation `uv run pytest -q` passed (`160 passed, 5 subtests passed in 19.00s`).
- 2026-04-19: tightened the training-entry architecture around the principle “one entrypoint, layered internals”. `train.py` now owns both public training entrypoints, so imaging orchestration sits beside classification orchestration in the unified CLI/lifecycle module, while `tasks.py` retains layered imaging builders and fit/evaluation helpers (`build_imaging_loaders`, `build_imaging_training_model`, `fit_imaging_model`) instead of exposing the public imaging training lifecycle directly. This keeps the user-facing entry surface unified without forcing low-level task implementation back into a single file. Verification: targeted entry-boundary tests passed (`5 passed, 155 deselected`), `py -3 -m compileall train.py tasks.py tests/test_d2nn_core.py` passed, and full validation `uv run pytest -q` passed (`160 passed, 5 subtests passed in 18.90s`). Final independent review returned `NO FINDINGS`.
- 2026-04-20: continued the unified-entrypoint / layered-internals alignment by making the imaging restore path match classification more closely. `tasks.fit_imaging_model()` now keeps an in-memory CPU copy of the best imaging state dict while still saving checkpoints on improvement, and `train.py` restores that cached best state before test evaluation whenever it is available instead of always reloading from disk. This removes one unnecessary post-training checkpoint read on the imaging path without changing CLI behavior or manifest semantics. TDD coverage was added in `tests/test_d2nn_core.py` to verify that `run_imaging_training()` prefers the cached best state before any disk reload. Verification: targeted regression test passed (`1 passed, 160 deselected`), `py -3 -m compileall train.py tasks.py tests/test_d2nn_core.py` passed, and full validation `uv run pytest -q` passed (`161 passed, 5 subtests passed in 21.95s`).
- 2026-04-20: follow-up review on the imaging restore-path alignment returned no findings. The only noted residual gap was missing direct coverage for the old fallback branch that reloads the best checkpoint from disk when no cached in-memory best state is available; that gap is now closed in `tests/test_d2nn_core.py`. Fresh verification after adding the fallback regression test: targeted imaging-entry tests passed (`4 passed, 158 deselected`), `py -3 -m compileall train.py tasks.py tests/test_d2nn_core.py` passed, and full validation `uv run pytest -q` passed (`162 passed, 5 subtests passed in 27.49s`).
- 2026-04-20: completed the last small restore-path alignment between imaging and classification. `tasks.fit_imaging_model()` now also reports `best_epoch`, and `train.py` skips any best-checkpoint restore on the imaging path when the final epoch is already the best epoch, matching the existing classification behavior. Together with the previous cached-best-state change, the imaging lifecycle is now `fit -> restore only if needed -> test -> save artifacts`, instead of always forcing a post-fit reload. TDD coverage now includes three imaging restore cases in `tests/test_d2nn_core.py`: cached-state restore, checkpoint fallback when no cache exists, and no restore when the final epoch is already best. Verification: targeted restore tests passed (`3 passed, 160 deselected`), `py -3 -m compileall train.py tasks.py tests/test_d2nn_core.py` passed, and full validation `uv run pytest -q` passed (`163 passed, 5 subtests passed in 26.49s`).
- 2026-04-20: addressed the actionable Claude Code review findings on the current unified-entrypoint refactor. Removed the duplicated `2026-04-19` architecture log entry, deleted the stale `tasks.py` imports left behind by the imaging-orchestration move, added `best_epoch` to the imaging manifest payload, and aligned task-helper boundaries by moving `fit_classification_model()` into `tasks.py` beside `fit_imaging_model()`. The imaging cache-path coverage gap is also closed: `tests/test_d2nn_core.py` now directly verifies `fit_imaging_model()` returns a CPU-cloned best state dict and that `run_imaging_training()` records `best_epoch` in the manifest. Verification: targeted review-follow-up tests passed (`6 passed, 159 deselected`), `py -3 -m compileall train.py tasks.py train_core.py tests/test_d2nn_core.py` passed, and full validation `uv run pytest -q` passed (`165 passed, 5 subtests passed in 19.71s`).
- 2026-04-20: completed a second Claude-review follow-up pass. Imaging training now fails fast on non-finite train/validation loss inside `tasks.fit_imaging_model()`, so the `best_epoch = 0` sentinel no longer leaks into imaging manifests or triggers a missing-checkpoint reload path. The task boundary is also cleaner: `print_model_summary()` now lives with the layered task helpers in `tasks.py`, `train_core.py` exposes a public `run_classification_epoch` alias, and the test suite now explicitly checks the imaging/classification summary wording split plus the absence of `tasks.run_imaging_training`. Verification after these fixes: targeted Claude-follow-up tests passed (`10 passed, 158 deselected`), `py -3 -m compileall train.py tasks.py train_core.py tests/test_d2nn_core.py` passed, and full validation `uv run pytest -q` passed (`168 passed, 5 subtests passed in 19.33s`).
- 2026-04-20: completed a third Claude-review follow-up pass focused on remaining hardening and symmetry gaps. Added explicit `unittest.mock` import, documented the `(best_epoch, best_state_dict)` restore contract on both task fit helpers, replaced the imaging finite checks with `math.isfinite`, synchronized the README imaging-manifest field summary to include `best_epoch`, and closed the restore-path test asymmetry by adding three classification restore tests plus the missing non-finite imaging `train_loss` test. Verification after these fixes: targeted symmetry tests passed (`10 passed, 162 deselected`), `py -3 -m compileall train.py tasks.py train_core.py tests/test_d2nn_core.py` passed, and full validation `uv run pytest -q` passed (`172 passed, 5 subtests passed in 18.69s`).
- 2026-04-22: continued the conservative code-denoise pass inside `artifacts.py` without changing file structure or public entrypoints. `OpticalConfig.with_overrides()` now uses `dataclasses.replace()` with filtered non-`None` overrides, `build_fabrication_readiness_summary()` delegates max/clipping details to same-file helpers, and `build_layer_stats()` delegates per-layer and optional thickness stats to same-file helpers. Added regression coverage for `None` override preservation, missing `thickness_map`, and empty fabrication summary arrays. Validation: targeted tests passed (`6 passed, 174 deselected`), `uvx ruff check .` passed, full `uv run python -m pytest -q` passed (`180 passed, 5 subtests passed`). Independent reviewer returned `No findings`.
